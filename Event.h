...
private:
    ...
    int MF; /*为1时说明这是一个同一时间段还有其他临时事件的临时事件*/
    int HF; /*与MF结合使用，为1时说明这个临时事件是这个时间段第一个添加的临时事件*/
...
private:
    ...
    //unordered_map<QString, int> namequeue;     //通过事件名字可以找到第一次出现在大课表events的位置
